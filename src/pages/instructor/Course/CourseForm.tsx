...
const CourseForm = () => {
  ...
  const location = useLocation(); // 👈 Access passed course data
  const course = location.state?.course; // 👈 Retrieve course from state

  console.log(course, "iam in the courseform page for editing, data here now");
  ...
  const handlePublish = async () => {
    if (formikRef.current) {
      ...
      formikRef.current.setTouched({
        ...
        price: true,
        thumbnail: true,
        lessons: formikRef.current.values.lessons.map((lesson: any) => ({
          title: true,
          description: true,
          video: lesson.video instanceof File,
          duration: true,
        })),
      });
      ...
      if (Object.keys(errors).length > 0) {
        toast.error(
          "Please provide all the required values before submitting."
        );
        return;
      }
      ...
      try {
        ...
        const uploadedLessons = await Promise.all(
          values.lessons.map(async (lesson: Lesson, index: number) => {
            if (lesson.video instanceof File) {
              ...
              // If existing video URL, return as is
              return lesson;
            }
            return lesson;
          })
        ...
        if (!results.payload.success) {
          toast.error(
            results.payload.message || "Course Adding failed,try again....."
          );
          return;
        }
        ...
        <Form className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-end space-x-4">
              ...
              <button
                type="button"
                onClick={() => navigate("/instructor/courses")} // 👈 Navigates back
                className="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50"
              >
                Cancel
              </button>
              ...
              >
                {isUploading ? (
                  <>
                    <span className="animate-spin h-5 w-5 mr-2 text-white">
                      ⏳
                    </span>
